...
<div class="bac_black input_print_form">
    <div class='message'>
        ...
        <form action='{{ route('account.organization.upload-seal.add') }}' method='post' enctype='multipart/form-data' class="form_settings">
            @csrf
        ...
        <form action='{{ route('account.organization.settings.update') }}' method='post' enctype='multipart/form-data' class="form_settings">
            ...
            <div class="flex_search_form">
                ...
                <div class="block_inpit_form_search">
                    ...
                    <input type="text" name='ogrn' value='{{ $user->ogrn }}'placeholder='ОГРНИП/ОГРН'>
                    @error('ogrn')
                        <div class='error-text'>{{ $message }}</div>
                    @enderror
                </div>
                <div class="block_inpit_form_search">
                    ...
                    <input type="text" name='organization' value='{{ $user->organization }}'placeholder='Организация'>
                    @error('organization')
                        <div class='error-text'>{{ $message }}</div>
                    @enderror
                </div>
...
<script>
    $( ".add_cemetery" ).on( "click", function() {
        ...
        $.ajax({
            type: 'POST',
            url: '{{ route("organization.add.cemetery") }}',
            data: {
                "_token": "{{ csrf_token() }}",
